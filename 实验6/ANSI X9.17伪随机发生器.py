...
def one_round(L, R, K):  # L 32bit字符串, R 32bit字符串, K 48bit字符串
    L, R = R, bin(int(S_Pbox(E(R, K)), base=2) ^ int(L, base=2))[2:].zfill(32)
    return L, R
...
def EDE(s, key1, key2):
    s = IP_trans(s, 1)
    s = DES(DES(DES(s, key1), key2), key1)
    s = IP_trans(s, 0)
    return s


def generate_key_stream(DT, V, key1, key2):
    _ = int(V, base=2) ^ int(EDE(DT, key1, key2), base=2)
    R = EDE(bin(_)[2:].zfill(64), key1, key2)
    _ = int(R, base=2) ^ int(EDE(DT, key1, key2), base=2)
    V = EDE(bin(_)[2:].zfill(64), key1, key2)
    return R, V


def main():
    V = bin(int(input().strip(), base=16))[2:].zfill(64)
    k1 = bin(int(input().strip(), base=16))[2:].zfill(64)
    k2 = bin(int(input().strip(), base=16))[2:].zfill(64)
    key1 = generate_key(k1)
    key2 = generate_key(k2)[::-1]
    n = int(input())
    result = [''] * n
    for i in range(n):
        DT = bin(int(input().strip(), base=16))[2:].zfill(64)
        R, V = generate_key_stream(DT, V, key1, key2)
        result[i] = R
    for R in result:
        print('0x' + hex(int(R, base=2))[2:].zfill(16))
