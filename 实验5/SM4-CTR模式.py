...
def CTR(s, IV, key, mode):  # s为任意长的字符串，去除0x  IV为整数
    text = []
    if mode == 1:  # 加密
        flag = 0
        for i in range(0, len(s) - 32, 32):  # 32*4=128bit 为SM4输入长度
            text.append(s[i:32 + i])
        if len(s) % 32 == 0:  # 不需要再填充
            text.append(s[len(s) - 32:])
        else:
            flag = (32 - len(s) % 32) // 2
            text.append(s[len(s) - len(s) % 32:])
        for item in text[:-1]:
            X = SM4(hex(IV)[2:].zfill(32), key)
            _ = hex(X ^ int(item, base=16))[2:].zfill(32)
            for i in range(16):
                print('0x' + _[i * 2: (i + 1) * 2], end=' ')
            print()
            IV += 1
        X = SM4(hex(IV)[2:].zfill(32), key)
        _ = hex(((X >> flag * 8) ^ (int(text[-1], base=16))) << flag * 8)[2:].zfill(32)
        for i in range(16 - flag):
            print('0x' + _[i * 2: (i + 1) * 2], end=' ')
        print()
    else:  # 解密
        for i in range(0, len(s) - 32, 32):
            text.append(s[i:32 + i])
        flag = 0
        if len(s) % 32 == 0:  # 不需要再填充
            text.append(s[len(s) - 32:])
