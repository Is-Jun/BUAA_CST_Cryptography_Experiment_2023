...
def OFB(n, s: str, IV: str, key, mode):  # n是字节，换成bit需要乘8
    text = []
    q, r = len(s) // (2 * n), (len(s) % (2 * n)) // 2  # r是字节需要整除2
    for i in range(q):
        text.append(s[2 * i * n:2 * (i + 1) * n])
    if r != 0:
        text.append(s[len(s) - 2 * r:])
    if mode == 1:
        cipher = ''
        for i in range(q):
            K = SM4(IV, key)
            K >>= (128 - n * 8)
            cipher += hex(int(text[i], base=16) ^ K)[2:].zfill(2 * n)
            IV = hex(((int(IV, base=16) << n * 8) & 0xffffffffffffffffffffffffffffffff) ^ K)[2:].zfill(32)
        if q != len(text):
            K = SM4(IV, key)
            K >>= (128 - r * 8)
            cipher += hex(int(text[-1], base=16) ^ K)[2:].zfill(2 * r)
        for i in range(0, len(cipher), 2):
            print('0x' + cipher[i:i + 2], end=' ')
            if i % 32 == 30:
                print()
    else:  # 实际上跟加密不能说很相似，只能说一模一样
        plaintext = ''
        for i in range(q):
            K = SM4(IV, key)
            K >>= (128 - n * 8)
            plaintext += hex(int(text[i], base=16) ^ K)[2:].zfill(2 * n)
            IV = hex(((int(IV, base=16) << n * 8) & 0xffffffffffffffffffffffffffffffff) ^ K)[2:].zfill(32)
        if q != len(text):
            K = SM4(IV, key)
            K >>= (128 - r * 8)
            plaintext += hex(int(text[-1], base=16) ^ K)[2:].zfill(2 * r)
        for i in range(0, len(plaintext), 2):
            print('0x' + plaintext[i:i + 2], end=' ')
            if i % 32 == 30:
                print()
