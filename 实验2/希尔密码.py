def extend_gcd(a, b):
    if b == 0:
        return 1, 0, a
    else:
        _x, _y, gcd = extend_gcd(b, a % b)
        x = _y
        y = _x - int(a // b) * _y
        if gcd < 0:
            gcd = -gcd
            x = -x
            y = -y
    return x, y, gcd


def mod_inv(a, n):
    x, y, gcd = extend_gcd(a, n)
    return x % n


def matrix_mul(matrix1, matrix2):
    n, m = len(matrix1), len(matrix1[0])
    matrix = []
    for i in range(n):
        list_ = []
        for j in range(m):
            list_.append(0)
        matrix.append(list_)

    for i in range(n):
        for j in range(m):
            for k in range(m):
                matrix[i][j] += matrix1[i][k] * matrix2[k][j]
            matrix[i][j] %= 26
    return matrix


def sub_matrix(matrix, i, j):
    # 矩阵matrix第i行第j列元素的余矩阵
    n, m = len(matrix), len(matrix[0])
    result = []
    # result = [[matrix[x][y] for y in range(m) if y != j] for x in range(n) if x != i]
    for x in range(n):
        list_ = []
        if x != i:
            for y in range(m):
                if y != j:
                    list_.append(matrix[x][y])
            result.append(list_)
    return result


def det_matrix(matrix):
    n, m = len(matrix), len(matrix[0])
    if n == 1 and m == 1:
        return matrix[0][0]
    else:
        value = 0
        for j in range(m):
            value += ((-1) ** (j + 2)) * matrix[0][j] * det_matrix(sub_matrix(matrix, 0, j))
        return value


def matrix_inv(matrix):
    n = len(matrix)
    result = []
    inv = mod_inv(det_matrix(matrix), 26)
    for i in range(n):
        list_ = []
        for j in range(n):
            list_.append(((-1) ** (i + j + 2)) * det_matrix(sub_matrix(matrix, j, i)) * inv % 26)
        result.append(list_)
    return result


def hill_cipher(n, k, s, way):
    if way == 1:
        cipher = ''
        m = len(s) // n
        for i in range(m):
            msg = [[]]
            for j in range(n):
                msg[0].append(ord(s[i * n + j]) - ord('a'))
            result = matrix_mul(msg, k)
            for j in result[0]:
                cipher += chr(j + ord('a'))
        return cipher
    else:
        plaintext = ''
        m = len(s) // n
        k_ = matrix_inv(k)
        # print(k_)
        # print(matrix_mul(k, k_))
        for i in range(m):
            msg = [[]]
            for j in range(n):
                msg[0].append(ord(s[i * n + j]) - ord('a'))
            result = matrix_mul(msg, k_)
            for j in result[0]:
                plaintext += chr(j + ord('a'))
        return plaintext



n = int(input())
k = []
for i in range(n):
    s = input()
    list_ = []
    for j in range(n):
        list_.append(int(s.split(' ')[j]))
    k.append(list_)
s = input().strip()
way = int(input())

print(hill_cipher(n, k, s, way))
